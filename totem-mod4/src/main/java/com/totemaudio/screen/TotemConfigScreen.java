...
public class TotemConfigScreen extends Screen {
    ...
    public TotemConfigScreen(Screen parent) {
        super(Component.literal("Totem Audio  00A77By: s1epyhead"));
        this.parent = parent;
    ...
    protected void init() {
        ...
        addRenderableWidget(Button.builder(Component.literal("\u21BA Reset to Defaults"),
                b -> { cfg.resetToDefaults(); rebuildWidgets(); })
                .bounds(cx, y, W, H).build()); y += GAP + 4;
            ...
            if (file != null && !file.isEmpty()) {
                ...
                try {
                    minecraft.level.playLocalSound(
                            ...
                            SoundEvents.TOTEM_USE, SoundSource.PLAYERS, vol, pit, false);
        ...
        new Thread(() -> {
            try {
                if (java.awt.Desktop.isDesktopSupported())
                    { java.awt.Desktop.getDesktop().open(dir.toFile()); return; }
            } catch (Throwable ignored) {}
            try { Runtime.getRuntime().exec(new String[]{"explorer", dir.toAbsolutePath().toString()}); }
            catch (Throwable ignored) {}
